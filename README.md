<h1 align="center">Hi, I'm Thalita 👋</h1>

<p align="center">
  <em>Full-Stack Developer building modern, scalable web apps</em>
</p>

## 👩‍💻 About Me

🌍 Based in **Switzerland**  
💬 Speak **English, Português, Español, Deutsch**  
💡 I enjoy building **real-world applications that solve practical problems**  
🤝 Strong believer in **teamwork, clean code, and continuous improvement**  
⚡ Always learning and pushing beyond my comfort zone

## 🛠️ Tech Stack

**Frontend:** React, Next.js, TypeScript, Tailwind  
**Backend:** Node.js, Express  
**Database:** MongoDB  
**Tools:** Git, GitHub, VS Code, Vite  

<div align="center">
  <img src="https://skillicons.dev/icons?i=js,ts,react,nextjs,nodejs,express,mongodb,html,css,tailwind,git,github,vscode,vite" />
</div>

## 📌 Featured Projects

### 🛒 Brew Commerce — Full-Stack E-Commerce Platform
A modern full-stack e-commerce application built with **Next.js 15, React, TypeScript, and MongoDB**.

⭐ **Key Features:**
- Complete shopping experience with product search & filters  
- Secure **Stripe checkout integration**  
- Admin dashboard for managing products & orders  
- Authentication with **JWT & bcrypt**  
- Email handling via Nodemailer  

🛠️ Tech: Next.js, React, TypeScript, Node.js, MongoDB, Stripe  
🔗 [Live Demo](https://brew-commerce.vercel.app) | [GitHub Repo](https://github.com/ThalitadosReis/brew-commerce)

---

### 🌙 Moonlight — Real-Time Event Discovery Platform
A full-stack MERN application for discovering and managing events in real time.

⭐ **Key Features:**
- Real-time chat using **Socket.io**  
- Event creation & management (full CRUD)  
- Location-based event search with **GeoJSON**  
- Image uploads via Cloudinary  
- User authentication system  

🛠️ Tech: React, Node.js, Express, MongoDB, Socket.io  
🔗 [GitHub Repo](https://github.com/orgs/JoTa-Events/repositories)

## 🔗 Connect with Me

[![Email](https://img.shields.io/badge/Email-dosreistha@gmail.com-D14836?style=for-the-badge&labelColor=101010)](mailto:dosreistha@gmail.com)
[![LinkedIn](https://img.shields.io/badge/LinkedIn-Thalita%20dos%20Reis-0077B5?style=for-the-badge&labelColor=101010)](https://www.linkedin.com/in/thalitadosreis/)
[![Portfolio](https://img.shields.io/badge/Portfolio-thalitadosreis.ch-F9A825?style=for-the-badge&labelColor=101010)](https://thalitadosreis.ch)
